...
class MemberController extends BaseController
{
    ...
    public function getPostsByMember(Request $request){
        $memberId = $request->getBody()['id'];
        //-------
        $user = $this->userService->getUserByID($memberId);
        global $globaluser;
        $globaluser = $user;
        //----------
        $posts = $this->postService->getPostsByMember($memberId);
        $member = $this->userService->getProfileUserByID($memberId);
        return $this->render('member/member-posts',[
            'member' => $member,
            'posts' => $posts,
            'css' => 'member-posts.css'
        ]);
    }
}